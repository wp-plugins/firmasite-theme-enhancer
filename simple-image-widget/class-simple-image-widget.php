...
class FirmaSite_Simple_Image_Widget extends WP_Widget {
	...
	function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		...
		$widget_options = wp_parse_args( $widget_options, array(
			...
			'description' => __( 'Display an image', "firmasite-theme-enhancer" ),
		) );
	...
	function render( $args, $instance ) {
		...
		$output = $args['before_widget'];
			...
			if ( $inside = apply_filters( 'FirmaSite_Simple_Image_Widget_output', '', $args, $instance, $this->id_base ) ) {
				$output .= $inside;
			} else {
				...
				// Add the image.
				if ( ! empty( $instance['image_id'] ) ) {
					$image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'FirmaSite_Simple_Image_Widget_output_default_size', 'medium', $this->id_base );
	...
	function form( $instance ) {
		...
		<div class="simple-image-widget-form">
			...
			<?php do_action( 'FirmaSite_Simple_Image_Widget_form_before', $instance, $this->id_base ); ?>

			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', "firmasite-theme-enhancer" ); ?></label>
			...
			<?php if ( ! is_FirmaSite_Simple_Image_Widget_legacy() ) : ?>
				<p class="simple-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image for the Widget', "firmasite-theme-enhancer" ); ?>"
					...
					<a href="#" class="<?php echo join( ' ', $button_class ); ?>"><?php _e( 'Choose an Image', "firmasite-theme-enhancer" ); ?></a>
				</p>
			<?php endif; ?>

			<?php if ( is_FirmaSite_Simple_Image_Widget_legacy() || ! empty( $instance['image'] ) ) : ?>
				<div class="simple-image-widget-legacy-fields">
					...
					<p>
						<label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php _e( 'Image URL:', "firmasite-theme-enhancer" ); ?></label>
					...
					<p>
						<label for="<?php echo $this->get_field_id( 'alt' ); ?>"><?php _e( 'Alternate Text:', "firmasite-theme-enhancer" ); ?></label>
				...
				foreach ( $fields as $field ) {
					switch ( $field ) {
						case 'image_size' :
							...
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e( 'Size:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'link' :
							...
							<p style="margin-bottom: 0.25em">
								<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e( 'Link:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'link_text' :
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'link_text' ); ?>"><?php _e( 'Link Text:', "firmasite-theme-enhancer" ); ?></label>
						...
						case 'text' :
							?>
							<p>
								<label for="<?php echo $this->get_field_id( 'text' ); ?>"><?php _e( 'Text:', "firmasite-theme-enhancer" ); ?></label>
	...
	function get_image_sizes( $image_id ) {
		...
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = FirmaSite_Simple_Image_Widget_Loader::get_image_size_names();
